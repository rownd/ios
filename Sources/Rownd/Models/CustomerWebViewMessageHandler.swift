...
class CustomerWebViewMessageHandler: NSObject, WKScriptMessageHandler {
    private var customerWebViewId: String
    
    init(customerWebViewId: String) {
        self.customerWebViewId = customerWebViewId
    }
    
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        guard let response = message.body as? String else { return }

        logger.trace("Received message from managed web view (\(self.customerWebViewId)): \(Redact.redactSensitiveKeys(in: response))")

        do {
            let hubMessage = try RowndCustomerInteropMessage.fromJson(message: response)

            logger.debug("Received message from customer web view with type: \(String(describing: hubMessage.type))")
            ...
            case .triggerSignInWithGoogle:
                ...
                if case .triggerSignInWithGoogle(let message) = hubMessage.payload {
                    ...
                    Rownd.googleSignInCoordinator.signIn(webViewId: self.customerWebViewId, iosClientId: signInWithGoogleMessage.iosClientId, serverClientId: signInWithGoogleMessage.serverClientId, intent: signInWithGoogleMessage.intent, hint: signInWithGoogleMessage.hint)
                } else {
