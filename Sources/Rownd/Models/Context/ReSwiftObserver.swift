...
public class ObservableState<T: Hashable>: ObservableObject, StoreSubscriber, ObservableSubscription
{
    ...
    func unsubscribe() {
        ...
        // CRITICAL: ReSwift's Store.subscriptions Set is not thread-safe.
        // Since deinit can be called from any thread, we must ensure unsubscribe
        // always executes on main thread to prevent concurrent modification crashes.
